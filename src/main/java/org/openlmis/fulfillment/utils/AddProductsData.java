...
public class AddProductsData {
    ...
    public AddProductsData(Document document, OrderProofOfDelivery orderProofOfDelivery)
            throws IOException {
        ...
        if (orderProofOfDelivery.getProfOfDeliveryLineItems() != null) {
            Map<String, List<OrderProofOfDeliveryLine>> orderProofofDeliveryLinesGrouped =
                    orderProofOfDelivery.getProfOfDeliveryLineItems().stream().collect(
            ...
            while (iterator.hasNext()) {
                ...
                List<OrderProofOfDeliveryLine> valList = orderProofofDeliveryLinesGrouped.get(key);
                for (OrderProofOfDeliveryLine item : valList) {
    ...
    private void addCellsToProductsTable(Table productsTable,
                                         OrderProofOfDeliveryLine item) throws IOException {
        Product currentProduct = item.getOrderLine().getProduct();
        Boolean fullSuplay = currentProduct.getFullSupply();
        Cell cell = getIcon(fullSuplay);
        productsTable.addCell(cell);
        productsTable.addCell(new Cell().add(new Paragraph(
    ...
    private Cell getIcon(Boolean fullSuplay) throws IOException {
        ...
        Cell cell = new Cell();
        if (fullSuplay == true) {
            cell.add(new Paragraph(new
                    Text("4").setFont(zapfdingbats).setFontSize(14).setTextAlignment(
                    TextAlignment.CENTER).setTextAlignment(TextAlignment.CENTER)));
        } else {
            cell.add(new Paragraph(new Text("5").setFont(zapfdingbats).setFontSize(14).setTextAlignment(
